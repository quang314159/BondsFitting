...
Version: 2.5 (Enhanced Precision)
"""
...
class RobustBootstrapSpotRates:
    ...
    def load_and_prepare_data(self):
        ...
        valid_bonds = (
            (self.bonds_df['Time_to_Maturity_Years'] > 0.01) &  # At least 3.65 days
            (self.bonds_df['Time_to_Maturity_Years'] < 50) &    # Less than 50 years
            (self.bonds_df['Clean_Price'] > 10) &               # Reasonable price range
            (self.bonds_df['Clean_Price'] < 200) &              # Reasonable price range
            (self.bonds_df['Coupon in %'] >= 0) &               # Non-negative coupon
            (self.bonds_df['Coupon in %'] <= 50) &              # Reasonable coupon
            (self.bonds_df['Coupon Frequency'].isin([1, 2, 4, 12])) &  # Standard frequencies
            (self.bonds_df['Annual Yield'] > 0) &               # Positive yield
            (self.bonds_df['Annual Yield'] < 50)                # Reasonable yield
        )
        ...
        try:
            with open(filename, 'w') as f:
                ...
                if self.nss_parameters:
                    ...
                    f.write(f"Clean data R²: {params['weighted_r_squared_clean']:.6f}\n")
                    f.write(f"All data R²: {params['weighted_r_squared_all']:.6f}\n")
                    f.write(f"Outliers excluded: {params['outlier_count']}\n")
                    f.write(f"Clean bonds used: {params['clean_count']}\n\n")
                    
                    f.write("NSS Parameters:\n")
                    f.write(f"  β₀: {params['beta0']*100:.3f}%\n")
                    f.write(f"  β₁: {params['beta1']*100:.3f}%\n")
                    f.write(f"  β₂: {params['beta2']*100:.3f}%\n")
                    f.write(f"  β₃: {params['beta3']*100:.3f}%\n")
                    f.write(f"  τ₁: {params['tau1']:.2f} years\n")
                    f.write(f"  τ₂: {params['tau2']:.2f} years\n")
...
def main():
    ...
    CSV_FILE_NAME   = "IBoxx_with_weights_310725.csv"   # <- change if your file name differs
    ...
    OUTLIER_THRESHOLD_BPS = 50.0  # Configurable outlier exclusion threshold
    ...
    try:
        ...
        calculator = RobustBootstrapSpotRates(csv_path, REPORT_DATE, OUTLIER_THRESHOLD_BPS)
